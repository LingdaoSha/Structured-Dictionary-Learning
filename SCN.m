function [sourcenorm]=SCN(source,Hta,Wta,Hso,filename)
...
% %%Lingdao modified 10/22/2015
% Ihat=Wta*Hso';

m = reshape(Hso,size(source,1),size(source,2),2);
for i = 1:2
   sourcenorm=uint8(255*exp(-m(:,:,i)));
   imwrite(sourcenorm,['NormalizedImage/','stain_',int2str(i),'_',filename],'compression','none');
end
